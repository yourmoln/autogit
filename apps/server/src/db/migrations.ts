...
const MIGRATIONS: Migration[] = [
  ...
  },
  {
    // Login gate.
    //
    // `auth_account` always holds exactly one row (`id = 'default'`), because
    // AutoGit is a single user tool: the credentials typed on the login page
    // are the ones the settings page can change later. Passwords are stored as
    // a scrypt hash, never as plain text.
    //
    // `auth_sessions` keeps the hashed session tokens handed out at login.
    // "保持登录" and plain sessions are stored the same way; `persistent` only
    // decides how long the token (and its cookie) lives.
    id: '004_auth_account',
    sql: `
