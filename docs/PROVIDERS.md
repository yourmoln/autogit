...
- 行内评审评论：AutoGit 只锚定「新文件」行号，并且只锚定本次 diff 里出现过的行（判定见 `util/diff-anchors.ts`）。锚点同时带上按 GitHub 口径算出的 patch 位置（第一个 `@@` 下面一行是 1，后续 hunk 头行各占一位）与「不数头行」的备选口径，供只认「diff 中的行数」的平台按顺序尝试。写入成功后还会读回核对锚点的真实行号（Gitea / Gitee 对行号的解释随版本变化）：锚定失败、请求被拒或读回对不上时，该条结论退回 PR 汇总评论并在任务日志里标注，不会丢失。
...
- 读回字段的语义已对照上游结构体确认：`CreatePullReviewComment.new_position` 的注释是 `if comment to new file line or 0`，而返回的 `PullReviewComment.position`（`LineNum`）填的是该评论所在行、`Line < 0` 的旧行评论才走 `original_position`（`modules/structs/pull_review.go`、`services/convert/pull_review.go`）。因此读回 `position` 与目标行号比较是同一把尺子。
- 行内评论写完会立刻读回 `GET /pulls/{n}/reviews/{id}/comments` 核对 `position`：实例若按别的口径解释 `new_position`（或干脆不返回该字段），对不上就删除这条 review（`DELETE /pulls/{n}/reviews/{id}`，Gitea 会连带删除它下面的代码评论；老版本没有该端点时按尽力而为忽略）并退回汇总评论——宁可少一条行内评论，也不留下错位的锚点。
...
- `position` 的语义在不同部署上不一致：官方 OpenAPI 文档（`/api/v5/doc_json`）把它描述成「PR代码评论diff中的行数」，返回体里另有 `new_line`；实际也有实例把它当新文件行号。AutoGit 把锚点里的两个 patch 位置口径与新文件行号依次试一遍（重复值只发一次）——**请求被拒（4xx）也会继续试下一种口径**，只有创建成功且读回 `new_line` 等于目标行号才算锚定成功；读回缺 `new_line`（或与目标行号不符）会删除刚创建的评论并退回汇总评论，避免留下错位的锚点。
